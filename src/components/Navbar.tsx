    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 font-medium text-foreground">
          ...
          <a href="#status" className="hover:text-primary transition-colors">{lang === 'ko' ? '해외 가맹현황' : 'Overseas Status'}</a>
          <a href="#process" className="hover:text-primary transition-colors">{lang === 'ko' ? '출점 프로세스' : 'Process'}</a>
          <a href="#contact" className="hover:text-primary transition-colors">{lang === 'ko' ? '문의' : 'Contact'}</a>
          <button 
